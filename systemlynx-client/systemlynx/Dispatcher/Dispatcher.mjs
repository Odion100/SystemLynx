...
export default function createDispatcher(_, systemContext) {
  ...
  Dispatcher.on = function (eventName, callback, { limit, interval, eventId } = {}) {
    ...
    // RFC 007: bind the handler to the module (so `this` is the module — consistent with methods and
    // middleware) when this dispatcher IS a module; fall back to systemContext otherwise.
    const bindTarget =
